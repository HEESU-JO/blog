...
- \( Q \in \mathbb{R}^{m \times m} \): **직교(orthogonal)** matrix
  → \( Q^T Q = I \), 열벡터들이 서로 직교하고 길이가 1 → orthonomal basis 
- \( R \in \mathbb{R}^{m \times n} \): **upper triangular** matrix  
  → 아래쪽이 모두 0 ( 조금 더 정확히 말하자면,ti,j가 R 행렬의 entry 일 때, 

QR 분해는 *“복잡한 행렬을 정리된 기저(Q)와 구조적 계수(R)로 바꿔놓는 과정”*이라고 이해하면 된다.

---

# 2. 왜 Q가 orthonom이고 R이 upper triangular matrix 이 되는가?
...
Gram–Schmidt process(GS)는 "orthonomal basis를 만드는 알고리즘"이고  
QR 분해는 이 과정을 *행렬 형태로 깔끔하게 정리한 것**이다.

정확히는:

1. Q의 열벡터는 GS로 얻은 orthonormal basis
2. R의 원소는 각 a_j가 q_i 방향으로 얼마만큼 투영되는지 나타낸 계수  
...
R이 upper triangular matrix이기 때문에 back-substitution으로 빠르고 안정적으로 풀 수 있음.

➡ 머신러닝 라이브러리에서도 내부적으로 QR을 많이 사용함.

---

## ⭐ 2) 정사영(projection) 계산

벡터 b를 A의 column space에 정사영하려면:
...
## ⭐ 3) 특잇값 분해(SVD) 계산의 전처리

SVD는 QR을 반복적으로 적용하는 알고리즘(Householder reduction) 위에서 돌아간다.  
즉, **QR → bidiagonalization → SVD 단계**로 이어지는 구조다.
...
1. 행렬 A를 QR로 분해해서  
2. 다시 \( A_{k+1} = R_k Q_k \)로 갱신하는 과정을 반복  

이게 **QR 알고리즘**이고,  
매우 많은 시스템에서 고유값 계산의 표준이다.

➡ MATLAB, NumPy, LAPACK 내부 알고리즘이 바로 이 방식 사용

---

## ⭐ 5) 머신러닝의 다양한 최적화 문제

- Orthogonalization 필요할 때  
- Normal Equation이 unstable할 때  
- RNN weight orthogonalization  
- PCA 이전 전처리 과정  
- compressed sensing 복원

QR은 “수치 안정성” 때문에 머신러닝에서 옛날부터 많이 쓰였다.
